...
RUN_USER_ID = integration_uuid("publish-normal-user")
RUN_WORKSPACE_ID = integration_uuid("publish-normal-workspace")
RUN_MEMBERSHIP_ID = integration_uuid("publish-normal-membership")
RUN_SOCIAL_ACCOUNT_ID = integration_uuid("publish-normal-social-account")
RUN_PROVIDER_ID = integration_token("publish-normal-page")
...
async def main():
    ...
    original_canary_mode_enabled = (
        publications.settings.meta_publish_canary_mode_enabled
    )
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            # v0.13F NORMAL MODE
            #
            # Existing CANARY issuance has already been proven.
            # Replace those grants with new activations issued
            # through the real HTTP endpoint while exact target
            # enforcement is disabled.
            #
            publications.settings.meta_publish_canary_mode_enabled = False
            publications.settings.meta_publish_canary_workspace_id = None
            publications.settings.meta_publish_canary_publication_id = None

            old_activation_values = tuple(
                activations.values()
            )

            for activation_value in old_activation_values:
            ...
            for activation_value in old_activation_values:
                if await redis_client.exists(
                    ...
                    raise AssertionError(
                        "old CANARY activation Redis key remains"
                    )
            ...
            try:
                for publication_id in publication_ids:
                    ...
                    if "no-store" not in cache_control:
                        raise AssertionError(
                            "NORMAL activation no-store missing"
                        )

                    body = response.json()

                    activation_value = body.get(
                        "activation"
                    )

                    if not activation_value:
                        raise AssertionError(
                            "NORMAL activation bearer missing"
                        )
                    ...
                    if (
                        ...
                        raise AssertionError(
                            "NORMAL activation content hash mismatch"
                        )
                    ...
                    if not await redis_client.exists(
                        ...
                        raise AssertionError(
                            "NORMAL activation Redis key missing"
                        )
            ...
            if len(activations) != 3:
                raise AssertionError(
                    "NORMAL activation count mismatch"
                )

            print(
                "NORMAL-mode real publish-activation issuance x3: PASS"
            )

            print(
                "NORMAL-mode activation target UNSET: PASS"
            )
            ...
            #
            # Temporarily restore CANARY policy for the existing
            # wrong-target /publish regression only.
            #
            ...
            print(
                "NORMAL-mode provider execution target UNSET: PASS"
            )
...
if __name__ == "__main__":
    ...
    print()
    print(
        "v0.13F NORMAL-MODE HTTP END-TO-END GATE: PASS"
    )
    print(
...
def test_v014_synthetic_identity_main_equivalence():
    ...
    async def run():
        ...
        unique = integration_token("publish-normal-fixture")
